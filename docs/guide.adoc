...
Aeroshift first tries to find the custom config in AeroShift locations:

. `~/.config/.aeroshift.toml`
. `~/.aeroshift.toml`
...
If no AeroShift config exists, Aeroshift falls back to upstream AeroSpace config locations:

. `~/.aerospace.toml`
. `+${XDG_CONFIG_HOME}/aerospace/aerospace.toml+`

If a config is found in more than one location within the active family then the ambiguity is reported.
...
If no custom config is found, Aeroshift will create `~/.config/.aeroshift.toml` from the bundled default config and load it.
...
----
mkdir -p ~/.config
cp /Applications/Aeroshift.app/Contents/Resources/default-config.toml ~/.config/.aeroshift.toml
----
